...
export default function Payments() {
  ...
  const buildStatementHtml = () => {
    ...
    *{box-sizing:border-box}body{font-family:Tahoma,'Sarabun',sans-serif;color:#111;margin:0;background:#fff}.page{width:210mm;min-height:297mm;margin:auto;padding:14mm}
    ...
    <tfoot><tr><td colspan="2">รวม</td><td class="num">${fmt(statementData.bills.reduce((s,d)=>s+Math.max(0,(d.total||0)-(d.vat||0)),0))}</td><td class="num">${fmt(statementData.bills.reduce((s,d)=>s+(d.vat||0),0))}</td><td class="num">${fmt(statementData.billed)}</td><td></td><td class="num">${fmt(statementData.received)}</td><td></td></tr><tr><td colspan="6">ยอดคงเหลือที่ยังไม่ได้จ่าย</td><td class="num closing">${fmt(statementData.closing)}</td><td></td></tr></tfoot></table></div></body></html>`
  }
