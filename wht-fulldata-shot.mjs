...
async function run() {
  ...
  const res = await page.evaluate(async () => {
    const r = await fetch('http://localhost:3737/api/withholding-tax', {
      ...
      body: JSON.stringify({
        company_id: 1, cert_no: 'DBG-FULL', book_no: '1', issue_date: '2026-06-15',
        form_type: 'nd53',
        ...
        items: [
          { income_type: '40_1', income_type_desc: '', pay_date: '2026-06-15', amount: 50000, tax_withheld: 5000 },
          { income_type: '40_2', income_type_desc: '', pay_date: '2026-06-15', amount: 10000, tax_withheld: 300 },
        ]
      })
    });
    return r.json();
  ...
  if (box) {
    // Top section
    await page.screenshot({ path: '/tmp/shots/form-top.png',
      clip: { x: box.x, y: box.y, width: box.w, height: box.h * 0.42 } });
    console.log('ss: form-top.png');

    // Income table
    await page.screenshot({ path: '/tmp/shots/form-income.png',
      clip: { x: box.x, y: box.y + box.h * 0.38, width: box.w, height: box.h * 0.44 } });
    console.log('ss: form-income.png');

    // Bottom
    await page.screenshot({ path: '/tmp/shots/form-bottom.png',
      clip: { x: box.x, y: box.y + box.h * 0.78, width: box.w, height: box.h * 0.22 } });
    console.log('ss: form-bottom.png');
  }

  // Clean up
  if (res?.id) await page.evaluate(async id => { await fetch(`http://localhost:3737/api/withholding-tax/${id}`, { method: 'DELETE' }); }, res.id);
  await app.close();
}
